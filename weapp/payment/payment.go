...
/**
请求前准备
o不是签名所需要的所有数据，od才是。两个是不同的对象，注意区分
1、准备签名数据signData
2. 判断其他选填的是否填了
3. 签名。也就是md5一下。字符串拼接在签名函数中完成的
 */
...
// @key payment secret key
	...
	if err != nil {
		fmt.Println("prepare reqData error:",err)
		return
	}
	fmt.Println("reqData:",reqData)
	// 此处就是统一下单了，把字符串凭借成xml在方法内部完成了
	data, err := util.PostXML(baseURL+unifyAPI, reqData)
	if err != nil {
		fmt.Println("PostXML error:",err)
		return
	}
	fmt.Println("PostXML data",data)
