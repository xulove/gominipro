...
func Login(appID, secret, code string) (lres LoginResponse, err error) {
	...
	if err != nil {
		fmt.Printf("code2url error:%v",err)
		return
	}

	res, err := http.Get(api)
	if err != nil {
		fmt.Printf("weapp:Login>http.get error:%v",err)
		return
	}
	...
	if res.StatusCode != 200 {
		...
		fmt.Printf("weapp:Login>res.StatusCode error:%v",err)
		return
	}
	...
	if err != nil {
		fmt.Printf("weapp:Login>json.NewDecoder error:%v",err)
		return
	}

	if data.Errcode != 0 {
		err = errors.New(data.Errmsg)
		fmt.Printf("weapp:Login>data.Errcode != 0 error:%v",err)
		return
	}
